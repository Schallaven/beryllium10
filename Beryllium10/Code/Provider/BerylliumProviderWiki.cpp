...
bool CBerylliumProviderWiki::SearchForCompound( const wxString searchtext, bool bExactMatch = false )
{
	...
	m_http.SetTimeout(30); // 30 Sekunden TimeOut
	...
	if ( iLanguage == wxLANGUAGE_ENGLISH )
	{
		...
		m_http.Connect( "en.wikipedia.org" ); request = "/wiki/Special:Export/%s";
	}

	// Ansonsten -> deutsche Wiki ist Standard.
	else
		m_http.Connect( "de.wikipedia.org" );
...
void CBerylliumProviderWiki::ParseInfoBoxSubstance( wxString szBlock )
{
	...
	while ( szParameter.length() > 0 )
	{
		...
		else if ( szParameter.compare("IUPACName") == 0 )
		{
			// Dinge zwischen {{ und }} entfernen
			RemoveBlockFromString( szValue, "{{", "}}" );
			RemoveBlockFromString( szValue, "<", ">" );
		...
		// Summenformel
		if ( szParameter.compare("Summenformel") == 0 )
		{
		...
		// Molare Masse
		if ( szParameter.compare("Molare Masse") == 0 )
		...
		// Siedepunkt
		if ( szParameter.compare("Siedepunkt") == 0 )
		{
			tempdata.szBoilingPoint = szValue.ToStdString();
		}
...
void CBerylliumProviderWiki::ParseInfoBoxElement( wxString szElement )
{
	...
	while ( szParameter.length() > 0 )
	{
		...
		if ( szParameter.compare("Name") == 0 )
			tempdata.SetNames( szValue.ToStdString() );
